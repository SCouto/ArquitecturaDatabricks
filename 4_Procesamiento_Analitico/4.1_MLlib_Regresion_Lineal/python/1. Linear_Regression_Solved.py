...
# MAGIC %md
# MAGIC # Preparing and cleaning Data

# COMMAND ----------

# DBTITLE 1,Cleaning from previous executions
output_path = 'dbfs:/FileStore/output/airbnb/clean_data/'
dbutils.fs.rm(output_path, True)

# COMMAND ----------

# DBTITLE 1,Run this cell without changes
...
fixed_price_df = raw_df.select(columns_to_keep).withColumn("price", translate(col("price"), "$,", "").cast("double"))
...
imputed_df.write.format('delta').mode('overwrite').save(clean_data_path)
...
# DBTITLE 1,Load Data
airbnb_df = spark.read.format("delta").load(clean_data_path)
display(airbnb_df)
