...
val eventStream = spark.readStream.option("maxFilesPerTrigger", 1)
  .schema(mySchema)
  .json(eventsPath)
...
eventStream.writeStream.format("memory").outputMode("append").queryName("eventsraw").start
...
val w = window($"time", "1 hour")
val myStream = eventStream.groupBy($"action",  w).count

// COMMAND ----------

dbutils.fs.rm("dbfs:/local_disk0/tmp", true)
myStream.writeStream.format("memory").outputMode("complete").queryName("myTable").start

// COMMAND ----------

display(spark.table("myTable").sort($"count".desc))
